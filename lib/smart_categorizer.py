"""
智能分类器
根据交易对方和交易类型智能识别分类
"""
import json
import os
from typing import Dict, Any, Optional

class SmartCategorizer:
    """智能分类器"""

    CORRECTIONS_FILE = os.path.join(os.path.dirname(__file__), 'corrections.json')
    _corrections: Dict[str, str] = {} # Class-level attribute to store corrections

    # 关键词映射规则 (保持不变)
    KEYWORD_RULES = {
        '餐饮': [
            '餐', '饭', '食', '吃', '外卖', '美团', '饿了么', '麦当劳', '肯德基',
            '星巴克', '瑞幸', '咖啡', '奶茶', '茶饮', '烧烤', '火锅', '小吃',
            '快餐', '面馆', '粉店', '饺子', '包子', '早餐', '午餐', '晚餐',
            '甜品', '蛋糕', '面包', '烘焙', '海底捞', 'KFC', '必胜客'
        ],
        '交通': [
            '交通', '出行', '滴滴', '打车', '代驾', '停车', '加油', '油费',
            '地铁', '公交', '共享单车', '骑行', '高铁', '火车', '机票', '航空',
            '汽车票', '过路费', 'ETC', '洗车', '12306', '携程', '去哪儿',
            '飞猪', '哈啰', '美团单车', '青桔', '轮胎'
        ],
        '购物': [
            '购物', '商城', '超市', '便利店', '淘宝', '天猫', '京东', '拼多多',
            '苏宁', '唯品会', '小米', '华为', '苹果', '服装', '鞋', '包',
            '化妆品', '护肤', '家电', '数码', '手机', '电脑', '耳机',
            '文具', '图书', '玩具', '日用品', '零食', '饮料', '7-11',
            '全家', '罗森', '屈臣氏', '沃尔玛', '家乐福', '盒马', '山姆',
            '得物', '李宁', '回力', 'mi店', 'su店', 'bj店', '银心铺',
            '帛诗', '柏丝', '猫人', '蓝色', '负一', '嘉', 'tb', 'yp店',
            '薛之', '铃铛', 'da舰', 'nak', 'pidan', '欧耐', '隔花', '夏金',
            'GS25', '7-Eleven', 'CU', 'e-mart', '韩国', '便利店'
        ],
        '娱乐': [
            '娱乐', '电影', '影院', '游戏', '网吧', 'Steam', '王者荣耀',
            'KTV', '酒吧', '健身', '游泳', '球馆', '台球', '剧本杀',
            '密室逃脱', '音乐', '演唱会', '话剧', '展览', '博物馆',
            '游乐场', '爱奇艺', '腾讯视频', '优酷', 'Netflix', 'QQ音乐',
            '网易云音乐', 'Spotify', 'Apple Music', 'B站', '哔哩哔哩',
            '完美世界', '四三九九', '国战传奇', '抖音生活服务'
        ],
        '固定支出': [
            '水电', '物业', '燃气', '宽带', '话费', '充值', '缴费',
            '电费', '水费', '暖气', '房租', '房东'
        ],
        '家用': [
            '家政', '维修', '装修', '家具', '家居',
            '床上用品', '厨具', '清洁', '洗衣', '电信', '移动', '联通'
        ],
        '医疗': [
            '医院', '诊所', '药店', '体检', '看病', '挂号', '药', '医疗',
            '牙科', '眼科', '美年大健康', '爱康', '瑞慈', '丁香', '平安好医生',
            '春雨医生', '疫苗', '保健品', '维生素'
        ],
        '学习办公': [
            '教育', '培训', '课程', '学费', '书籍', '文具', '办公',
            '打印', '复印', '知网', '得到', '喜马拉雅', '网课', '在线教育',
            '英语', '驾校', '证书', '考试', '报名费', 'Coursera', 'Udemy',
            '腾讯课堂', '网易云课堂', '考研', '公考', '深度求索', '百度',
            '阿里云', '云服务', '域名', '服务器', 'RACKNERD', '宁波市奉化区黑方体'
        ],
        '理发美容': [
            '理发', '美发', '造型', '剪发', '染发', '烫发', 'INSTYLE',
            'XZHAIRSALON', '工作室', '美容', '美甲', '按摩', 'SPA',
            '凯旋', '阿泽工作室', '3AM'
        ],
        '家庭支出': [
            '笨蛋蛋', '蛋蛋', 'の紾', '妈妈', '母亲', '爸爸', '父亲',
            '给妈妈', '给爸爸', '给对象', '给家人'
        ],
        '红包转账': [
            '红包', '转账', '提现', '充值', '退款', '还款', '借款',
            '小荷包', '支付宝小荷包', '微信红包'
        ],
        '贷款': [
            '贷款', '花呗', '借呗', '信用卡', '分期', '中融小额贷款'
        ],
        '其他收入': [
            '工资', '薪水', '奖金', '报销', '退税', '理财', '股票', '基金', '利息',
            '分红', '佣金', '稿费', '兼职', '补贴', '红包收入'
        ]
    }

    # 特殊交易对方映射(精确匹配优先) (保持不变)
    EXACT_MERCHANT_MAP = {
        # 家庭支出
        '笨蛋蛋🥚': '家庭支出',
        'の紾...x.マ': '家庭支出',
        '培': '家庭支出',  # 可能是家人
        '伟': '家庭支出',  # 可能是家人/朋友

        # 固定支出(房租等)
        '房东': '固定支出',
        '你有本事偷能量 你有本事浇水啊': '固定支出',
        '上海公共支付一网通办': '固定支出',  # 政务缴费

        # 娱乐
        'Steam': '娱乐',
        '完美世界': '娱乐',
        '四三九九网络股份有限公司': '娱乐',
        '国战传奇': '娱乐',
        '北京卡路里科技有限公司': '娱乐',  # 健身APP
        '西安奥体中心商业租赁': '娱乐',  # 体育场馆

        # 贷款
        '深圳市中融小额贷款有限公司': '贷款',

        # 购物
        '得物': '购物',
        'pidan Gallery': '购物',
        '阿里巴巴1688plus会员': '购物',
        '上海格物致品网络科技有限公司': '购物',
        '麦狄店': '购物',
        'su猫': '购物',
        'nak': '购物',
        'yp店': '购物',
        'da舰': '购物',
        '常州店': '购物',
        'bu4': '购物',
        '洺豪店': '购物',
        'YHJM': '购物',

        # 餐饮
        '饿了么': '餐饮',
        '老张特色面': '餐饮',
        '小马炒鸡拌面': '餐饮',
        '抖音生活服务商家': '餐饮',  # 抖音团购餐饮
        '商家优惠': '餐饮',  # 通常是餐饮优惠券
        '丁美女': '餐饮',
        '陈鹏3': '餐饮',
        '爽': '餐饮',
        '黄旭东': '餐饮',
        '杰': '餐饮',
        '汪芬': '餐饮',

        # 交通
        '风神轮胎': '交通',

        # 旅游
        '苏州程会玩国际旅行社有限公司': '旅游',
        '智慧旅游商务科技有限公司': '旅游',

        # 韩国消费(旅游/购物)
        'GS25 Yeondongga': '购物',
        'GS25 linked hap': '购物',
        '7-Eleven Seogwi': '购物',
        'CU Aewoljaundan': '购物',
        'e-mart new jeju': '购物',
        'SAEBYOUL HEYYO': '餐饮',
    }

    # 支付宝分类映射 (保持不变)
    ALIPAY_CATEGORY_MAP = {
        '餐饮美食': '餐饮',
        '交通出行': '交通',
        '公共交通': '交通',
        '日用百货': '购物',
        '购物': '购物',
        '服饰美容': '购物',
        '数码电器': '购物',
        '娱乐': '娱乐',
        '运动健康': '娱乐',
        '生活服务': '家用',
        '充值缴费': '家用',
        '家居家装': '家用',
        '医疗健康': '医疗',
        '教育学习': '学习办公',
        '金融保险': '其他',
        '红包转账': '红包转账',
        '其他': '其他'
    }

    # 微信分类映射 (保持不变)
    WECHAT_CATEGORY_MAP = {
        '商户消费': '购物',
        '扫二维码付款': '餐饮',
        '微信红包': '红包转账',
        '转账': '红包转账',
        '二维码收款': '其他收入',
        '退款': '红包转账',
        '理财通': '其他'
    }

    @classmethod
    def _clean_counterparty(cls, counterparty: str) -> str:
        """
        清理交易对方名称, 移除括号内容和特殊字符
        """
        if not counterparty:
            return ''
        clean_counterparty = counterparty.split('(')[0].split('（')[0].strip()
        clean_counterparty = clean_counterparty.replace('*', '').strip()
        return clean_counterparty

    @classmethod
    def _load_corrections(cls):
        """从 corrections.json 加载修正数据"""
        if not os.path.exists(cls.CORRECTIONS_FILE):
            # If file doesn't exist, initialize empty corrections and return
            cls._corrections = {}
            return

        try:
            with open(cls.CORRECTIONS_FILE, 'r', encoding='utf-8') as f:
                content = f.read()
                if not content: # Handle empty file case
                    cls._corrections = {}
                else:
                    cls._corrections = json.loads(content)
        except json.JSONDecodeError:
            print(f"Warning: {cls.CORRECTIONS_FILE} is malformed. Initializing empty corrections.")
            cls._corrections = {}
        except Exception as e:
            print(f"Error loading {cls.CORRECTIONS_FILE}: {e}. Initializing empty corrections.")
            cls._corrections = {}

    @classmethod
    def _save_corrections(cls):
        """保存修正数据到 corrections.json"""
        try:
            with open(cls.CORRECTIONS_FILE, 'w', encoding='utf-8') as f:
                json.dump(cls._corrections, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"Error saving {cls.CORRECTIONS_FILE}: {e}")

    @classmethod
    def add_correction(cls, counterparty: str, final_category: str):
        """
        添加或更新一个修正记录。
        这应该由外部调用，当用户手动修正了一个分类时。
        """
        cleaned_counterparty = cls._clean_counterparty(counterparty)
        if cleaned_counterparty:
            cls._corrections[cleaned_counterparty] = final_category
            cls._save_corrections()

    @classmethod
    def categorize(cls, source_type, category, counterparty, is_income):
        """
        智能分类
        :param source_type: 来源类型 'alipay' 或 'wechat'
        :param category: 原始交易分类
        :param counterparty: 交易对方
        :param is_income: 是否收入
        :return: 分类结果
        """
        # 1. 优先检查修正记录
        cleaned_counterparty = cls._clean_counterparty(counterparty)
        if cleaned_counterparty and cleaned_counterparty in cls._corrections:
            return cls._corrections[cleaned_counterparty]

        # 收入类型单独处理
        if is_income:
            # 检查关键词
            for keyword in cls.KEYWORD_RULES.get('其他收入', []):
                if keyword in category or keyword in counterparty:
                    return '其他收入'
            return '工作收入'

        # 2. 优先精确匹配交易对方 (从原有逻辑中迁移，并使用清理后的交易对方)
        if counterparty in cls.EXACT_MERCHANT_MAP: # Use original counterparty for exact map
            return cls.EXACT_MERCHANT_MAP[counterparty]

        # 3. 尝试基于原始分类映射
        mapped = None
        if source_type == 'alipay':
            for key, val in cls.ALIPAY_CATEGORY_MAP.items():
                if key in category:
                    mapped = val
                    break
        else:  # wechat
            for key, val in cls.WECHAT_CATEGORY_MAP.items():
                if key in category:
                    mapped = val
                    break

        # 4. 基于交易对方关键词进行二次分类
        combined_text = f"{category} {counterparty}"
        keyword_match = None
        max_matches = 0

        for cat, keywords in cls.KEYWORD_RULES.items():
            if cat == '其他收入':  # 跳过收入类
                continue

            matches = sum(1 for kw in keywords if kw in combined_text)
            if matches > max_matches:
                max_matches = matches
                keyword_match = cat

        # 5. 如果关键词匹配置信度高,使用关键词分类
        if max_matches >= 2 or (max_matches >= 1 and not mapped):
            return keyword_match

        # 6. 否则使用原始分类映射结果
        return mapped if mapped else '其他'

    @classmethod
    def generate_note(cls, category, counterparty, final_category):
        """
        生成智能备注
        :param category: 原始分类
        :param counterparty: 交易对方
        :param final_category: 最终分类
        :return: 备注文本
        """
        clean_counterparty = cls._clean_counterparty(counterparty) # Reuse clean_counterparty

        # 如果交易对方太长,截取关键部分
        if len(clean_counterparty) > 15:
            clean_counterparty = clean_counterparty[:15]

        # 组合备注: 最终分类-交易对方
        if clean_counterparty and clean_counterparty != category:
            note = f"{final_category}-{clean_counterparty}"
        else:
            note = category

        return note[:50]

# Load corrections when the class is imported
SmartCategorizer._load_corrections()
